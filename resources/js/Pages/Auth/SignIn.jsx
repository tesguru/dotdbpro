    ...
    post('/sign-in', {
      ...
      onError: (errors) => {
        if (errors.email_address) {
          toast.error('Sign in failed', errors.email_address);
        } else if (errors.password) {
          toast.error('Sign in failed', errors.password);
        } else {
      ...
      <div className={`min-h-[calc(100vh-180px)] transition-colors duration-500 flex items-center justify-center p-4 ${
        ...
        <div className={`w-full max-w-md rounded-3xl border backdrop-blur-xl transition-colors duration-500 ${
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold mb-2">Welcome to Dnwhouse</h1>
            <p className={`text-sm ${isDark ? 'text-purple-200' : 'text-gray-600'}`}>
              Sign in to discover amazing domains
            </p>
          </div>
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              />
              <label htmlFor="remember" className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                Remember me
              </label>
            </div>
